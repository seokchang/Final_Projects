	...
	<div class="notice-content">
		...
		<div class="notice-naeyong">
			<table class="notice-tbl">
				...
				<tr>
					<th>작성자</th>
					<td><a href="javascript:void(0);" class="dm-a"
						title="클릭하여 쪽지를 보내보세요!" data-toggle="modal" data-target="#myModal">${b.userId }</a></td>
				...
				<tr>
					<th>첨부파일</th>
					<td colspan="3"><a
						href="javascript:fileDownload('${b.boardFilename }','${b.boardFilepath }')">${b.boardFilepath }</a>
					</td>
				...
				<c:if test="${b.boardFilename!=null }">
					<tr>
						<td colspan="4" style="border-bottom: none;">
							<div
								style="width: 80%; margin: 0 auto; margin-top: 10px; text-align: center;">
								<div
									style='width: 400px; margin: 0 auto; margin-bottom: 10px; margin-top: 20px;'>
									<img src="/resources/upload/board/${b.boardFilepath }"
										style="width: 400px;">
								</div>
							</div>
						</td>
				...
				<tr>
					<td colspan="4">${b.boardContents }</td>
				</tr>
				
				<c:if
					test="${sessionScope.user.userId.equals(b.userId) && sessionScope.user!=null}">
					<tr>
						<td colspan="4" style="text-align: center; height: 55px;"><a
							href="/boardUpdateFrm.do?boardNo=${b.boardNo }"
							class="notice-btn">수정</a> <a
							href="/deleteBoard.do?boardNo=${b.boardNo }" class="notice-btn">삭제</a>
						</td>
					</tr>
				</c:if>
			</table>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body">
							수신자 아이디 <input type="text" name="dmReceiver" id="dmReceiver"
								value="${b.userId }"><br> <br>
						...
						<div class="modal-footer">
							<button class="write-btn"
								onclick="insertDm('${sessionScope.user.userId }')">보내기</button>
						</div>
		...
		<c:if test="${sessionScope.user!=null }">
			<div class="inputComment">
				<form action="/insertBoardComment.do" method="post">
					<ul>
						<li><input type="hidden" name="boardCommentLevel" value="1">
							<input type="hidden" name="boardCommentWriter"
							value="${sessionScope.user.userId }"> <input
							type="hidden" name="boardRef" value="${b.boardNo }">
		...
		<div class="commentList">
			<c:forEach items="${list }" var="bc">
				<ul>
					...
					<li>
						<p>${bc.boardCommentContent }</p> <textarea
							name="boardCommentContent" class="changeComment"
							style="display: none;">${bc.boardCommentContent }</textarea> <c:if
							test="${sessionScope.user!=null }">
							<p class="linkBox">
								<c:if
									test="${bc.boardCommentWriter.equals(sessionScope.user.userId) }">
									<a href="javascript:void(0)" class="modify-btn"
										onclick="modifyComment(this,'${bc.boardCommentNo }','${b.boardNo }')">수정</a>
									<a href="javascript:void(0)" class="delete-btn"
										onclick="deleteComment(this,'${bc.boardCommentNo }','${b.boardNo }')">삭제</a>
								</c:if>
							</p>
